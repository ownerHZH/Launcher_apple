...
import com.mkl.launcher.HandleView;
import com.mkl.launcher.LauncherSettings.BaseLauncherColumns;
import com.mkl.launcher.R;
import com.mkl.launcher.ShortcutInfo;
import com.mkl.launcher.DropTarget.DragObject;
...
public final class Launcher extends Activity implements View.OnClickListener,
	...
	private static final String TOOLBAR_ICON_METADATA_NAME = "com.mkl.launcher.toolbar_icon";
...
//			setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);
	...
	private boolean completeAdd(PendingAddArguments args) {
		...
		// In any situation where we have a multi-step drop, we should reset the
		// add info only after
...
//		setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
	...
	private void restoreState(Bundle savedState) {
		...
		if (mAppsCustomizeTabHost != null) {
			...
			if (curTab != null) {
				...
				// is set
				mAppsCustomizeContent.setContentType(mAppsCustomizeTabHost
	...
	private void setupViews() {
		...
		// add by luoguoliang
		//mDockbar = (DockBar)mDragLayer.findViewById(R.id.dockbar);  
		...
		// add by luoguoliang
		//dragController.addDragListener(mDockbar);
	...
	public int getDockIconSize(){
		...
		mDockIconSize=r.getDimensionPixelSize(R.dimen.app_icon_size);
		mDockIconSize = (int)(mDockIconSize*scale);
		return mDockIconSize;
	}
    ...
    { 
		...
		final Animation myAnimation_Romate = AnimationUtils.loadAnimation(mContext,R.anim.scale); 
 	    final Animation myAnimation_Translate = AnimationUtils.loadAnimation(mContext,R.anim.translate);
 	    iconsize =getDockIconSize();
		ImageView imageView =new ImageView(mContext);	
		Bitmap bm = shortcutInfo.mIcon;
		if (bm == null) {
			// come from apps
			bm = shortcutInfo.getIcon(mIconCache);
		}
		BitmapDrawable bd=new BitmapDrawable(bm);
		imageView.setImageDrawable(Utilities.drawReflection(bd, this));
		//imageView.setImageBitmap(shortcutInfo.mIcon);
		imageView.setOnClickListener(this);
		imageView.setFocusable(true);
		imageView.setTag(shortcutInfo);
		imageView.setMinimumWidth(iconsize);		
		//mDockbar.setOnHover(imageView);
		imageView.setOnHoverListener(new OnHoverListener(){
            ...
            {       		        		
            	...
            	case MotionEvent.ACTION_HOVER_ENTER:           		
                         ...
                         anim.addAnimation(myAnimation_Romate);
                         anim.addAnimation(myAnimation_Translate);
	...
	int[] getSpanForWidget(ComponentName component, int minWidth,
			int minHeight, int[] spanXY) {
		if (spanXY == null) {
			spanXY = new int[2];
		}

		Rect padding = AppWidgetHostView.getDefaultPaddingForWidget(this,
				component, null);
		...
		return CellLayout.rectToCell(getResources(), requiredWidth,
				requiredHeight, null);
	}
	...
	int[] getMinResizeSpanForWidget(AppWidgetProviderInfo info, int[] spanXY) {
		return getSpanForWidget(info.provider, info.minResizeWidth,
				info.minResizeHeight, spanXY);
	}

	int[] getSpanForWidget(PendingAddWidgetInfo info, int[] spanXY) {
		return getSpanForWidget(info.componentName, info.minWidth,
				info.minHeight, spanXY);
	}
	...
	private void completeAddAppWidget(final int appWidgetId, long container,
		...
		if( appWidgetInfo == null)
			appWidgetInfo = staticAppWidgetProviderInfo;
	...
	void closeSystemDialogs() {
		...
		/**
		 * We should remove this code when we remove all the dialog code. try {
		 * dismissDialog(DIALOG_CREATE_SHORTCUT); // Unlock the workspace if the
		 * dialog was showing } catch (Exception e) { // An exception is thrown
		 * if the dialog is not visible, which is fine }
		 * 
		 * try { dismissDialog(DIALOG_RENAME_FOLDER); // Unlock the workspace if
		 * the dialog was showing } catch (Exception e) { // An exception is
		 * thrown if the dialog is not visible, which is fine }
		 */
...
//			appSearchData.putString(Search.SOURCE, "launcher-search");
	...
	void addAppWidgetFromPick(Intent data) {
		...
		if (LOGD)
			Log.d(TAG, "dumping extras content=" + data.getExtras());
		addAppWidgetImpl(appWidgetId, null);
	}

	AppWidgetProviderInfo  staticAppWidgetProviderInfo = null;
	void addAppWidgetImpl(int appWidgetId, PendingAddWidgetInfo info) {
		AppWidgetProviderInfo appWidget = mAppWidgetManager
				.getAppWidgetInfo(appWidgetId);
		appWidget = info.info;
		staticAppWidgetProviderInfo = appWidget;
		Log.i("dige"," "+appWidgetId+" /"+appWidget);
		Log.i("dige"," "+info.info.label);
	...
	void addAppWidgetFromDrop(PendingAddWidgetInfo info, long container,
		...
		AppWidgetManager.getInstance(this).bindAppWidgetIdIfAllowed(appWidgetId,
				info.componentName);
...
//				}
	...
	private void growAndFadeOutFolderIcon(FolderIcon fi) {
		...
		ObjectAnimator oa = ObjectAnimator.ofPropertyValuesHolder(fi, alpha,
				scaleX, scaleY);
		...
		if (allowLongPress && !mDragController.isDragging()) {
			if (itemUnderLongClick == null) {
				...
				if (!(itemUnderLongClick instanceof Folder)) {
					...
					Log.i("cheng","Launcher onLongClick");
		...
		private void changeFolderName() {
			...
			if (!TextUtils.isEmpty(name)) {
				...
				if (mWorkspaceLoading) {
					...
					if (folderIcon != null) {
						// At some point we'll probably want some version
						// of setting
	 ...
	 * app - Go back with home key - Go back with back key  make this not
	 ...
	 * screen.
	 */
	private void showAppsCustomizeHelper(boolean animated,
		...
		if (animated) {
			final ValueAnimator scaleAnim = ValueAnimator.ofFloat(0f, 1f)
			...
			scaleAnim.addListener(new AnimatorListenerAdapter() {
				...
				@Override
				public void onAnimationEnd(Animator animation) {
					// If we don't set the final scale values here, if this
					// animation is cancelled
					// it will have the wrong scale value and subsequent
					// cameraPan animations will
					// not fix that
					toView.setScaleX(1.0f);
					toView.setScaleY(1.0f);
					if (toView instanceof LauncherTransitionable) {
						((LauncherTransitionable) toView)
								.onLauncherTransitionEnd(instance, scaleAnim,
										false);
					}
			...
			if (toView instanceof LauncherTransitionable) {
				...
				delayAnim = lt.onLauncherTransitionStart(instance,
						mStateAnimation, false);
			}
			// if the anim is delayed, the LauncherTransitionable is responsible
			// for starting it
			if (!delayAnim) {
				// TODO: q-- what if this anim is cancelled before being
				// started? or started after
				// being cancelled?
		...
		if (animated) {
			...
			alphaAnim.addListener(new AnimatorListenerAdapter() {
				...
				public void onAnimationEnd(Animator animation) {
					...
					if (fromView instanceof LauncherTransitionable) {
						((LauncherTransitionable) fromView)
								.onLauncherTransitionEnd(instance, alphaAnim,
										true);
					}
	...
	void showAllApps(boolean animated) {
		...
		showAppsCustomizeHelper(animated, true);
		...
		if (mQsbDivider != null && mDockDivider != null) {
			...
			if (animated) {
				...
				mDividerAnimator.playTogether(
						ObjectAnimator.ofFloat(mQsbDivider, "alpha", 1f),
						ObjectAnimator.ofFloat(mDockDivider, "alpha", 1f));
				mDividerAnimator.setDuration(mSearchDropTargetBar
	...
	private void updateTextButtonWithDrawable(int buttonId,
			Drawable.ConstantState d) {
		TextView button = (TextView) findViewById(buttonId);
		button.setCompoundDrawables(d.newDrawable(getResources()), null, null,
				null);
	}
	...
	public void bindItems(ArrayList<ItemInfo> shortcuts, int start, int end) {
        ...
        Log.i(TAG,"bindItems");
		final Workspace workspace = mWorkspace;
		//final DockBar dockbar = mDockbar;		
			...
			case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:				
				if (container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
					...
					workspace.addInScreen(shortcut, item.container,
							item.screen, item.cellX, item.cellY, 1, 1, false);
				} else if (container == LauncherSettings.Favorites.CONTAINER_DOCKBAR) {		
					Log.i(TAG,"item.screen=="+item.screen+"  item.ToString"+item.toString());
					//dockbar.addItemAt(item, item.screen);
				}
				break;
			case LauncherSettings.Favorites.ITEM_TYPE_FOLDER:				
				if (container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
					FolderIcon newFolder = FolderIcon.fromXml(R.layout.folder_icon,
							...
							mIconCache);
					workspace.addInScreen(newFolder, item.container,
							item.screen, item.cellX, item.cellY, 1, 1, false);
				} else if (container == LauncherSettings.Favorites.CONTAINER_DOCKBAR) {
					try {
						//dockbar.addItemAt(item, item.screen);
...
//		int naturalOri = Configuration.ORIENTATION_LANDSCAPE;
		int naturalOri = Configuration.ORIENTATION_PORTRAIT;
...
//					setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);
	...
	/* Cling related */
	private static final String PREFS_KEY = "com.mkl.launcher.prefs";
			...
			anim.addListener(new AnimatorListenerAdapter() {
				public void onAnimationEnd(Animator animation) {
					...
					SharedPreferences prefs = getSharedPreferences(
							"com.mkl.launcher.prefs", Context.MODE_PRIVATE);
	...
	public void showFirstRunWorkspaceCling() {
		...
		SharedPreferences prefs = getSharedPreferences(PREFS_KEY,
	...
	public void showFirstRunAllAppsCling(int[] position) {
		...
		SharedPreferences prefs = getSharedPreferences(PREFS_KEY,
				Context.MODE_PRIVATE);
		if (isClingsEnabled()
				&& !prefs.getBoolean(Cling.ALLAPPS_CLING_DISMISSED_KEY, false)) {
	...
	public Cling showFirstRunFoldersCling() {
		...
		Cling cling = null;
		if (isClingsEnabled()
				&& !prefs.getBoolean(Cling.FOLDER_CLING_DISMISSED_KEY, false)) {
...
interface LauncherTransitionable {
	...
	boolean onLauncherTransitionStart(Launcher l, Animator animation,
			boolean toWorkspace);

	void onLauncherTransitionEnd(Launcher l, Animator animation,
			boolean toWorkspace);
}
