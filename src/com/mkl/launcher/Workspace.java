...
import com.mkl.launcher.FolderIcon.FolderRingAnimator;
import com.mkl.launcher.InstallWidgetReceiver.WidgetMimeTypeHandlerData;
...
public class Workspace extends SmoothPagedView implements DropTarget,
	...
	public Workspace(Context context, AttributeSet attrs, int defStyle) {
		...
		if (LauncherApplication.isScreenLarge()) {
			...
			// This code currently fails on tablets with an aspect ratio <
		...
		Log.d(TAG, "---- cellCountX : " + cellCountX);
		Log.d(TAG, "---- cellCountY : " + cellCountY);
...
//	}
	...
	private float wallpaperOffsetForCurrentScroll() {
		...
		// wallpaper will move
		// at this orientation. On tablets in portrait mode we don't move all
		// the way to the
...
//		float adjustedScrollX = Math.max(0, Math.min(mScrollX, mMaxScrollX));
		float adjustedScrollX = Math.max(0, Math.min(1, mMaxScrollX));
...
//					getMeasuredHeight());
			mBackground.setBounds(1, 0, 1 + getMeasuredWidth(),
					getMeasuredHeight());
	...
	// This method just applies the value mChildrenLayersEnabled to all the
	// pages that
...
//					ViewConfiguration.ALPHA_THRESHOLD
					if (page.getVisibility() == VISIBLE
							&& page.getAlpha() > ViewConfiguration.getTapTimeout()) {
//						((ViewGroup) getPageAt(i))
//								.setChildrenLayersEnabled(true);
						((ViewGroup) getPageAt(i)).setClickable(true);
//						.setChildrenLayersEnabled(true);
					}
				}
			}
...
//			final int paddingBottom = mPaddingBottom + offset;
			...
			final int offset = (height - pageHeight - 1 - 1) / 2;
			final int paddingTop = 1 + offset;
			final int paddingBottom = 1 + offset;
...
//								- paddingBottom);
				d.setBounds(1, paddingTop,
						1 + d.getIntrinsicWidth(), height
								- paddingBottom);
				d.draw(canvas);
...
//						paddingTop, mScrollX + width, height - paddingBottom);
				d.setBounds(1 + width - d.getIntrinsicWidth(),
						paddingTop, 1 + width, height - paddingBottom);
				d.draw(canvas);
			}
		}
	}
...
//					((ViewGroup) getChildAt(i)).setChildrenLayersEnabled(false);
	...
	void changeState(final State state, boolean animated, int delay) {
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			float finalAlpha = (!mFadeInAdjacentScreens || stateIsSpringLoaded || (i == mCurrentPage)) ? 1f
					: 0f;
			...
			// Update the rotation of the screen (don't apply rotation on Phone
			// UI)
			if (LauncherApplication.isScreenLarge()) {
				if (i < mCurrentPage) {
		...
		if (animated) {
			...
			animWithInterpolator.addListener(new AnimatorListenerAdapter() {
				...
				public void onAnimationEnd(android.animation.Animator animation) {
					// The above code to determine initialAlpha and finalAlpha
					// will ensure that only
					// the current page is visible during (and subsequently,
					// after) the transition
					// animation. If fade adjacent pages is disabled, then
					// re-enable the page
					// visibility after the transition animation.
					if (!mFadeInAdjacentScreens && stateIsNormal
							&& oldStateIsSmall) {
						for (int i = 0; i < getChildCount(); i++) {
							final CellLayout cl = (CellLayout) getChildAt(i);
							cl.setAlpha(1f);
					...
					.addUpdateListener(new LauncherAnimatorUpdateListener() {
						public void onAnimationUpdate(float a, float b) {
							...
							for (int i = 0; i < getChildCount(); i++) {
								final CellLayout cl = (CellLayout) getChildAt(i);
								cl.invalidate();
								cl.setTranslationX(a * mOldTranslationXs[i]
										+ b * mNewTranslationXs[i]);
								cl.setTranslationY(a * mOldTranslationYs[i]
										+ b * mNewTranslationYs[i]);
								cl.setScaleX(a * mOldScaleXs[i] + b
										* mNewScaleXs[i]);
								cl.setScaleY(a * mOldScaleYs[i] + b
										* mNewScaleYs[i]);
			...
			rotationAnim
					.addUpdateListener(new LauncherAnimatorUpdateListener() {
						public void onAnimationUpdate(float a, float b) {
							...
							for (int i = 0; i < getChildCount(); i++) {
								final CellLayout cl = (CellLayout) getChildAt(i);
								cl.setRotationY(a * mOldRotationYs[i] + b
										* mNewRotationYs[i]);
							}
						}
					});
...
//		xy[1] = xy[1] + mScrollY - v.getTop();
		...
		xy[0] = xy[0] + 1 - v.getLeft();
		xy[1] = xy[1] + 1 - v.getTop();
...
//		xy[1] -= (mScrollY - v.getTop());
	...
	private float[] getDragViewVisualCenter(int x, int y, int xOffset,
		...
		res[1] = top + dragView.getDragRegion().height() / 2 -15;
	...
	private void onDropExternal(final int[] touchXY, final Object dragInfo,
		...
		if (info instanceof PendingAddItemInfo) {
			...
			float dragViewScale = Math.min(
					r.width() / d.dragView.getMeasuredWidth(), r.height()
	...
	public void onDropMoving(int x,int y){
